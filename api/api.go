...
func registerRoutes(r *gin.Engine, config *config.Config) {
	...
	v1 := api.Group("/v1")
	{
		...
		//Company
		company := v1.Group("/companies", middlewares.Authentication(config), middlewares.Authorization([]string{constatns.AdminRole}))
		routes.Companies(company, config)

		//CarType
		carType := v1.Group("/car-types", middlewares.Authentication(config), middlewares.Authorization([]string{constatns.AdminRole}))
		routes.CarTypes(carType, config)
		...
		//CarModelYear
		carModelYear := v1.Group("/car-model-years", middlewares.Authentication(config), middlewares.Authorization([]string{constatns.AdminRole}))
		routes.CarModelYear(carModelYear, config)

		//PersianYear
		persianYear := v1.Group("/persian-years", middlewares.Authentication(config), middlewares.Authorization([]string{constatns.AdminRole}))
		routes.PersianYear(persianYear, config)
	}
}
